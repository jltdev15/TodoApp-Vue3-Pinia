...
export const useTaskStore = defineStore("todosSchool", () => {
  // States
  const taskList = ref([]);
  const countTask = ref("");
  ...
  const saveLocalStorage = async (data) => {
    localStorage.setItem("SchoolTaskList", JSON.stringify(data));
  };
  ...
  const getDataLocalStorage = () => {
    const data = JSON.parse(localStorage.getItem("SchoolTaskList"));
    console.log(data);
    // console.log(data);
    taskList.value = data;
  };
