...
export const useTaskStore = defineStore("todosWork", () => {
  // States
  const taskList = ref([]);
  const countTask = ref("");
  ...
  const saveLocalStorage = async (data) => {
    localStorage.setItem("WorkTaskList", JSON.stringify(data));
  };
  ...
  const getDataLocalStorage = () => {
    const data = JSON.parse(localStorage.getItem("WorkTaskList"));
    console.log(data);
    // console.log(data);
    taskList.value = data;
  };
